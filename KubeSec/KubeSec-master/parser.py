...
def getKeyRecursively(dict_, list2hold, depth_=0):
    ...
    if isinstance(dict_, dict):
        ...
        for key_, val_ in sorted(
            dict_.items(), key=lambda x: x[0] if (isinstance(x[0], str)) else str(x[0])
        ):
...
def checkIfValidK8SYaml(path2yaml):
    ...
    # Early reject: input must be a string path
    if not isinstance(path2yaml, str):
        print(f"Skipping invalid path input type: {type(path2yaml)}")
        return False

    # Early reject: path must exist and be a file
    if not os.path.isfile(path2yaml):
        print(f"Path does not exist or is not a file: {path2yaml}")
        return False

    try:
        dict_as_list = loadMultiYAML(path2yaml)

        # Additional validation: must load something
        if not isinstance(dict_as_list, list) or not dict_as_list:
            return False

        yaml_dict = getSingleDict4MultiDocs(dict_as_list)
        if not isinstance(yaml_dict, dict) or not yaml_dict:
            return False
        ...
        temp_ = []
        for k_ in k_list:
            if isinstance(k_, (list, tuple)) and k_:
                temp_.append(k_[0])
        ...
        print(f"Caught exception for path={path2yaml}: {e}")
        val2ret = False
...
def checkIfValidHelm(path_script):
    ...
    ) and (constants.VALUE_KW in path_script):
        val_ret = True
    return val_ret
...
def find_json_path_keys(json_file, parent_path="", paths=None):
    ...
    if isinstance(json_file, dict):
        for key, value in json_file.items():
            ...
            if (
                isinstance(key, ruamel.yaml.comments.CommentedKeyMap) and value is None
        ...
        if isinstance(key_jsonpath_mapping[key], list):
            for i in key_jsonpath_mapping[key]:
                ...
                result = subprocess.check_output(
                    ["yq", yq_parameter, filepath], universal_newlines=True
                )
                ...
                for line in output:
                    ...
                    if line_convert > 0:
                        line_number = line_convert + count_initial_comment_line(
                            filepath
                        )
    ...
    for dic in lis_dic:
        ...
        if ((dic is None) == False) and (isinstance(dic, dict)):
            ...
            for k_ in keys4dic:
                if k_ in key_lis:
                    dict2ret[
                        k_ + constants.DOT_SYMBOL + constants.YAML_DOC_KW + str(counter)
                    ] = dic[k_]
                else:
                    key_lis.append(k_)
                    dict2ret[k_] = dic[k_]
